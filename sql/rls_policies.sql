...
-- user_data(user_id uuid, account_id uuid) must exist

CREATE POLICY leads_rls ON public.leads
  USING (EXISTS (SELECT 1 FROM user_data ua WHERE ua.user_id = auth.uid() AND ua.account_id = leads.account_id));

CREATE POLICY msgs_in_rls ON public.messages_in
  USING (EXISTS (
    SELECT 1 FROM leads l JOIN user_data ua ON ua.account_id = l.account_id
...
CREATE POLICY kb_articles_rls ON public.account_kb_articles
  USING (EXISTS (SELECT 1 FROM user_data ua WHERE ua.user_id = auth.uid() AND ua.account_id = account_kb_articles.account_id));

CREATE POLICY kb_embeddings_rls ON public.kb_embeddings
  USING (EXISTS (SELECT 1 FROM user_data ua WHERE ua.user_id = auth.uid() AND ua.account_id = kb_embeddings.account_id));
