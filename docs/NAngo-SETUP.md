...
- `NEXT_PUBLIC_NANGO_HOST` - Nango API host (optional, defaults to `https://api.nango.dev`)

### **Note:**
- **Public keys are deprecated** - Nango now uses short-lived Connect session tokens instead
- The session token is automatically generated by your backend (`/api/crm/session-token`)
- No public key needed - the frontend receives a session token and uses it directly
...
3. Copy your **Secret Key** from the dashboard (Settings → Secret Key)
4. **Note:** Public keys are no longer needed - Nango uses session tokens instead
