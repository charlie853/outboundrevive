      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-52 flex-shrink-0 border-r border-surface-border flex flex-col bg-surface-bg/50">
          <div className="p-4 border-b border-surface-border">
            <div className="flex items-center gap-2 text-lg font-semibold text-ink-1">
              <Inbox className="w-5 h-5 text-brand-600" />
              Inbox
            </div>
          </div>
          <nav className="p-2 flex flex-col gap-0.5">
            <a className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-ink-2 hover:bg-surface-card hover:text-ink-1 transition" href="#">
        ...
        {/* Center: email list */}
        <div className="w-[420px] flex-shrink-0 border-r border-surface-border flex flex-col bg-surface-card">
          <div className="border-b border-surface-border px-4 py-4 space-y-3">
            <h2 className="text-lg font-semibold text-ink-1">Your Replies ({filteredThreads.length})</h2>
            <p className="text-sm text-ink-2 mt-0.5">All replies from your cold email campaigns in one place.</p>
            <label className="flex items-center gap-2 text-sm text-ink-2 cursor-pointer">
              <input type="checkbox" checked={showUnreadOnly} onChange={(e) => setShowUnreadOnly(e.target.checked)} className="rounded border-surface-border text-brand-600 focus:ring-brand-500" />
              Show only unread
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-lg border border-surface-border text-sm text-ink-1 placeholder:text-ink-3 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 bg-white"
              />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-6 space-y-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="h-20 animate-pulse rounded-xl bg-surface-bg" />
                ))}
              </div>
            ) : error ? (
              <div className="rounded-xl border border-rose-200 bg-rose-50 mx-4 mt-4 px-4 py-3 text-sm text-rose-700">
                {error}
                <button type="button" className="ml-3 rounded-lg border border-rose-300 px-3 py-1 text-xs font-medium text-rose-700 hover:bg-rose-100" onClick={() => { setError(null); loadThreads(); }}>Retry</button>
              </div>
            ) : pageThreads.length === 0 ? (
              <div className="p-8 text-center">
                <p className="text-ink-2">No replies yet.</p>
                <p className="text-sm text-ink-2 mt-1">Replies from your campaigns will appear here when leads respond.</p>
              </div>
            ) : (
                  ...
                  return (
                    <li key={t.id}>
                      <button
                        ...
                        className={`w-full text-left px-4 py-4 flex gap-3 hover:bg-surface-bg/80 transition ${isSelected ? 'bg-brand-50 border-l-4 border-brand-600' : ''}`}
                      >
                        ...
                        <Star className="w-4 h-4 text-ink-3 flex-shrink-0 mt-1" aria-hidden />
                      </button>
                    </li>
                  );
                })}
          ...
          {filteredThreads.length > PAGE_SIZE && (
            <div className="px-4 py-3 border-t border-surface-border flex items-center justify-between text-sm text-ink-2">
              <span>{page * PAGE_SIZE + 1}-{Math.min((page + 1) * PAGE_SIZE, filteredThreads.length)} of {filteredThreads.length}</span>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} className="p-1.5 rounded-lg hover:bg-surface-bg disabled:opacity-40 text-ink-2" aria-label="Previous page">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button type="button" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="p-1.5 rounded-lg hover:bg-surface-bg disabled:opacity-40 text-ink-2" aria-label="Next page">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-surface-bg">
          {!threadId ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-lg font-semibold text-ink-1">Select an email to read</p>
            ...
            <div className="mx-4 mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
              {threadError}
              <button type="button" className="ml-3 rounded-lg border border-rose-300 px-3 py-1 text-xs font-medium text-rose-700 hover:bg-rose-100" onClick={() => threadId && loadThreadDetail(threadId)}>Retry</button>
            </div>
          ) : thread ? (
            <>
              ...
              <div className="border-t border-surface-border bg-surface-card p-6">
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm text-ink-1 min-h-[120px] focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                  rows={4}
                />
                <button
                  type="button"
